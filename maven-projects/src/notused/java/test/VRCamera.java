 ...
 * {@link Camera#far} and offset will be overwritten
 * on every call to {@link #update()} based on the tracked values
 * from the head mounted display. The offsets
...
{
	public final ManagerOpenVR managerOpenVR;
	public final Eye eye;
	public final Matrix4 eyeSpace = new Matrix4();
	public final Matrix4 invEyeSpace = new Matrix4();


	public VRCamera( ManagerOpenVR managerOpenVR,Eye eye )
	{
		this.managerOpenVR = managerOpenVR;
		this.eye = eye;
	}
	...
	public void update( boolean updateFrustum )
	{
		...
		Utils.hmdMat4toMatrix4( projectionMat,projection );

		// get the eye space matrix from the HDM
		VRSystem.VRSystem_GetEyeToHeadTransform( eye.index,eyeMat );
		Utils.hmdMat34ToMatrix4( eyeMat,eyeSpace );
		invEyeSpace.set( eyeSpace ).inv();

		// get the pose matrix from the HDM
		VRDevice hmd = managerOpenVR.getDeviceByType( VRDeviceType.HeadMountedDisplay );
